...
public class TestCSVParser {
    ...
    public void testContractParser() {
        Contract contract = new Contract(0, 123L, 456L,
                new Client(0, "name", 0L, Gender.FEMALE, "12356789")
        );
        ...
        assertEquals(contract.getBeginDate(), contractConstructed.getBeginDate());
        assertEquals(contract.getOwner().getFullName(),
                contractConstructed.getOwner().getFullName()
        );
        assertEquals(contract.getOwner().getGender(), contractConstructed.getOwner().getGender());
    }
    ...
    public void testContractFromCSV() {
        ...
        assertEquals(123, contract.getBeginDate().intValue());
        assertEquals("name", contract.getOwner().getFullName());
    }
